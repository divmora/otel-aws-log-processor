...
func (p *WAFProcessor) Process(ctx context.Context, logger *slog.Logger, s3Client *s3.Client, bucket, key string) ([]LogAdapter, error) {
	// Extract common attributes from S3 key
	accountID, region := utils.ParseRegionAccountFromS3Key(key)

	return ReadAndParseJSONFromS3(ctx, logger, s3Client, bucket, key, p.MaxBatchSize, p.MaxConcurrent, func(data []byte) (LogAdapter, error) {
