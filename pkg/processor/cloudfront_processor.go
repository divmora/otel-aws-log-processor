...
func (p *CloudFrontProcessor) Process(ctx context.Context, logger *slog.Logger, s3Client *s3.Client, bucket, key string) ([]LogAdapter, error) {
	...
	if strings.HasSuffix(key, ".parquet") {
		return ReadAndParseParquetFromS3(ctx, logger, s3Client, bucket, key, p.MaxBatchSize, p.MaxConcurrent, func(row *parser.CloudFrontParquetLogEntry) (LogAdapter, error) {
			if row == nil {
				return nil, nil
			}
